...
const OnlineOrders = () => {
  ...
  useEffect(() => {
    const fetchCustomersAndSalesmen = async () => {
      try {
        const response = await fetch(`${baseURL}/get/account-details`);
        if (!response.ok) {
        ...
        // Filter customers (CUSTOMERS and SUPPLIERS)
        const filteredCustomers = result.filter(
          (item) =>
            item.account_group === "CUSTOMERS" ||
            item.account_group === "SUPPLIERS",
        ...
        // Filter salesmen (SALESMAN)
        const filteredSalesmen = result.filter(
          (item) => item.account_group === "SALESMAN",
        );
        setSalesmen(filteredSalesmen);

        console.log("Salesmen loaded:", filteredSalesmen); // Debug log
    ...
    fetchCustomersAndSalesmen();
  }, []);



// Update the handleSalesmanChange function in SalesForm.js
const handleSalesmanChange = (e) => {
  const salesmanId = e.target.value;
  
  console.log("Selected Salesman ID:", salesmanId); // Debug log
  
  if (salesmanId) {
    const selectedSalesman = salesmen.find(
      (salesman) => String(salesman.account_id) === String(salesmanId)
    );
    
    console.log("Found Salesman:", selectedSalesman); // Debug log
    
    if (selectedSalesman) {
      setFormData((prev) => ({
        ...prev,
        salesman_id: String(selectedSalesman.account_id), // Ensure it's a string
        salesman_name: selectedSalesman.account_name,
      }));
      console.log("FormData after selection:", formData); // Debug log
    }
  } else {
    setFormData((prev) => ({
      ...prev,
  ...
  // Add this useEffect to calculate total commission on all items
useEffect(() => {
  // Calculate total price from all repairDetails
  const totalPrice = repairDetails.reduce((sum, item) => {
  ...
  // Update formData with the total commission amount
  setFormData(prev => ({
    ...prev,
    salesman_commission_amount: calculatedCommission
  }));
  
}, [repairDetails, formData.salesman_commission]);
...
const handleAdd = () => {
    ...
    ...repairDetails,
    {
      ...
      rate:
        formData.rate && parseFloat(formData.rate) > 0
          ...
          : "",
      imagePreview: formData.imagePreview,
      // PRESERVE SALESMAN COMMISSION VALUES
      salesman_id: formData.salesman_id || null,
      salesman_name: formData.salesman_name || null,
      salesman_commission: formData.salesman_commission || 0,
      salesman_commission_amount: 0, // Will be recalculated by useEffect
    },
  ];
  ...
  // Reset form data but PRESERVE salesman fields
  setFormData((prevData) => ({
    ...prevData,
    disscount: "",
    disscount_percentage: "",
    ...
    // Don't set commission amount here - it will be recalculated by useEffect
  }));
...
const handleDelete = (indexToDelete) => {
  if (window.confirm("Are you sure you want to delete this item?")) {
    const updatedDetails = repairDetails.filter((_, index) => index !== indexToDelete);
    setRepairDetails(updatedDetails);
    // Commission will be recalculated by useEffect
...
const handleSave = async () => {
  ...
  try {
    ...
    // Calculate total commission from all items before saving
    const totalPrice = repairDetails.reduce((sum, item) => {
      return sum + (parseFloat(item.total_price) || 0);
    }, 0);
    const commissionPercent = parseFloat(formData.salesman_commission) || 0;
    const totalCommissionAmount = (totalPrice * commissionPercent) / 100;

    // Prepare payload with salesman fields
    const dataToSave = {
      repairDetails: repairDetails.map((item) => ({
        ...item,
        invoice_number: updatedFormData.invoice_number,
        ...
        salesman_commission_amount: totalCommissionAmount, // Use calculated total
      })),
      totalAmount: totalAmount,
      discountAmt: discountAmt,
      festivalDiscountAmt: festivalDiscountAmt,
        ...
        <Form>
          ...
          <div className="sales-form" style={{ marginTop: "-40px" }}>
            ...
            <div className="sales-form-right">
              <InvoiceDetails formData={formData} setFormData={setFormData} salesmen={salesmen}
