...
const fetchCompanies = async () => {
  try {
    ...
    const firstCompany = response.data && response.data.length > 0 ? response.data[0] : null;
    setCompany(firstCompany);
    console.log("firstCompany=", firstCompany);
  } catch (error) {
    console.error("Error fetching companies:", error);
    setCompany(null); // Set to null instead of keeping previous state
    setError(error.message);
  } finally {
    setLoading(false);
  }
...
const handleSave = async () => {
  ...
  try {
    ...
    // Generate PDF - Add safety checks for company and product
    const pdfDoc = (
      <PDFLayout
        formData={updatedFormData}
        repairDetails={repairDetails}
        ...
        netAmount={netAmount}
        netPayableAmount={netPayableAmount}
        company={company || {}} // Provide empty object as fallback
        product={product || []} // Provide empty array as fallback
      />
    );
