  ...
  useEffect(() => {
    if (!isNetAmtManual) {
      setFormData((prevData) => {
        ...
        // Making Charges Calculation - Based on Gross Weight
        if (updatedFormData.Making_Charges_On === "MC / Gram") {
          // Calculate MC based on Gross Weight (not total_pure_wt)
          const grossWeightForMC = parseFloat(updatedFormData.gross_weight) || 0;
          updatedFormData.total_mc = (makingChargesValue * grossWeightForMC).toFixed(2);
        } else if (updatedFormData.Making_Charges_On === "MC / Piece" && updatedFormData.gross_weight > 0) {
          // Calculate MC per gram based on gross weight
          updatedFormData.Making_Charges_Value = (totalMC / (parseFloat(updatedFormData.gross_weight) || 1)).toFixed(2);
        } else if (updatedFormData.Making_Charges_On === "MC %") {
          // MC % remains based on total_amount
          updatedFormData.total_mc = ((makingChargesValue * updatedFormData.total_amount) / 100).toFixed(2);
        }
