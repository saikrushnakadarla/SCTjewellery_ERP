...
function Salesmanmaster() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ensure account_group is explicitly set to 'SALESMAN'
      const dataToSend = {
        ...formData,
        account_group: 'SALESMAN' // Explicitly set this
      };
      ...
      alert(`Salesman ${id ? "updated" : "created"} successfully!`);
      
      navigate(location.state?.from || "/salesmantable", {
  ...
  const handleBack = () => {
    const from = location.state?.from || "/salesmantable";
    navigate(from);
  };
          ...
          <Row>
            ...
            <Col md={12}>
              <div className="image-upload-section">
                <h5>Upload Salesman Documents/Photos</h5>
                <input
                  type="file"
                  ref={fileInputRef}
                  style={{ display: 'none' }}
